...
// BLUE ZONE ESP state - populated from the game's own BroadcastCircleInfo event (see the
// hkProcessEvent hook), the same replicated sync the game uses to draw its own minimap
// circle, rather than anything we compute ourselves. BlueCircleSync.Z is assumed to carry
// the current radius (the function has no separate radius parameter) - confirmed/adjust via
// the debug readout in DrawEsp if the drawn circle doesn't match the real one in a live match.
float g_ZoneCenterX = 0.0f, g_ZoneCenterY = 0.0f, g_ZoneRadius = 0.0f;
float g_ZoneTimeAtSync = 0.0f;
time_t g_ZoneSyncRealTime = 0;
int g_ZoneState = 3; // ECircleInfo::NoCircleInfo until the first broadcast arrives
...
bool g_FlashPatchesApplied = false;  // tracks whether Flash patches are currently applied - prevents re-applying every frame
float FastMovementScale = 1.0f;  // proportional engine speed scale - disabled from default 1.3x to normal speed (no automatic speedup)
float LongHandReach = 100.0f;    // extended melee/reach distance - see case 850 in run_thread
bool ScopeZoomBoost = true;       // always on - no panel toggle, see the SCOPE ZOOM comment below
float ScopeZoomTarget = 4.0f;     // desired zoom multiplier while boost is active - see DrawEsp
int ScopeZoomRedDotID = 203001;   // confirmed via an actual screenshot of the debug readout -
int ScopeZoomHoloID = 203002;     // the earlier hand-typed "2030001/2" had an extra digit
...
// CREATE_COLOR macro removed - was causing memory leak (new without delete) and unused in code
...
return s;}
std::string getStringName(SDK::UObject *Object) {
std::string s;
for (auto super = Object->ClassPrivate; super; super = (SDK::UClass *) super->SuperStruct) {
if (!s.empty())
s += ".";
        ...
        auto UTAMA_xD = UTAMA[i];
          //  auto Actor = Actors[i];
            ...
            if (UTAMA_xD->IsA(ASTExtraPlayerCharacter::StaticClass()))
            {
                ...
                // Was tracing from the calculated weapon muzzle position, which sits further
                // back than the camera/eye during a wall-peek - so it could disagree with what
                // ESP's visibility color already showed (that one traces from the controller's
                // own eye view, see IsLineOfSightTo/DrawEsp). Switched to the exact same check
                // ESP uses so both agree: if ESP shows an enemy as visible, aimbot now targets
                // them too, instead of refusing shots ESP itself says are already lined up.
                if (Config.AimBot.VisCheck) {
                    if (!IsLineOfSightTo((uintptr_t)Player))
                    continue;
                }
...
ImColor 粉红 = ImColor(ImVec4(220/255.f, 108/255.f, 150/255.f, 0.95f));
...
case 4451:
if (UTAM_FLY){
long p1 = getPointer(UE4 + 0X5090DC8); if (!p1) break;
long p2 = getPointer(p1 + 0x20); if (!p2) break;
long p3a = getPointer(p2 + 0x18); if (!p3a) break;
long c1 = getPointer(p3a + 0x1C8)+0x3C8;
long p3b = getPointer(p2 + 0x2CC); if (!p3b) break;
long c3 = p3b + 0x60;
long p3c = getPointer(p3b + 0x310); if (!p3c) break;
long c2 = getPointer(p3c) + 0x1AC;
writefloat(c2, 99999);
writefloat(c1, 1.15);
writefloat(c3, 1.35);
} else {
...
long p2 = getPointer(p1 + 0x20); if (!p2) break;
long p3a = getPointer(p2 + 0x18); if (!p3a) break;
long d1 = getPointer(p3a + 0x1C8)+0x3C8;
long p3b = getPointer(p2 + 0x2CC); if (!p3b) break;
long d3 = p3b + 0x60;
long p3c = getPointer(p3b + 0x310); if (!p3c) break;
long d2 = getPointer(p3c) + 0x1AC;
long p4 = getPointer(UE4 + 0x50B7600); if (!p4) break;
long d4 = getPointer(p4 + 0xA8)+0x30;
writefloat(d2, 8192);
writefloat(d4,-980);
writefloat(d1, 1);
writefloat(d3, 1);
...
case 4449:
if(UTAM_FLASH){
if (!g_FlashPatchesApplied) {  // only apply patches once, not every frame
try {
long a1 = getPointer(UE4 + 0x50A5EAC) +0x20;//145
long a2 = getPointer(UE4 + 0x50A5EAC) +0x24;//145
long p1 = getPointer(UE4 + 0x4F08EB8); if (!p1) break;
long p2 = getPointer(p1 + 0xA8); if (!p2) break;
long a3 = getPointer(p2 + 0x24C)+0x24;
long p3 = getPointer(UE4 + 0X5090DC8); if (!p3) break;
long p4 = getPointer(p3 + 0x20); if (!p4) break;
long p5 = getPointer(p4 + 0x2CC); if (!p5) break;
long p6 = getPointer(p5 + 0x310); if (!p6) break;
long a4 = getPointer(p6) + 0x138;
long p7 = getPointer(p4 + 0x18); if (!p7) break;
long a5 = getPointer(p7 + 0x1C8)+0x3C8;
long a6 = p5 + 0x60;
long a7 = p5 + 0x14F4;
long a8 = p5 + 0x14F0;
long a9 = p5 + 0x1558;// fix
if (!a1 || !a2 || !a3 || !a4 || !a5 || !a6 || !a7 || !a8 || !a9) break;
writefloat(a1,165);
writefloat(a2,165);
...
g_FlashPatchesApplied = true;
} catch (...) {
g_FlashPatchesApplied = false;
UTAM_FLASH = false;
}
}
} else {
if (g_FlashPatchesApplied) {  // only restore patches once, when toggling off
// Flash disabled - restore all patches (reverting the game to normal state)
...
// This ran as an unthrottled busy-spin (no sleep at all) - a background thread hammering
// memory writes as fast as possible steals CPU time from the game's own main/render
// thread, which showed up as the character "freezing" for a few seconds. 10ms is more
// than fast enough to keep re-asserting the pinned height against normal game gravity/
// physics ticks (which run at 30-60Hz) while using a tiny fraction of the CPU a bare
// spin loop did.
usleep(10000);
}}
break;
...
Config.PlayerESP.BlueZone = true;
}else{
...
Config.PlayerESP.BlueZone = false;
...
// HIGH DAMAGE: unlike Long Hand, everything this writes (the fixed UE4+0x50A5EAC damage
// table address, the libUE4.so byte patches, the kFox-found values) is global game data -
// none of it depends on which pawn instance you're currently controlling. The old retrigger
// condition (fire whenever the localPlayer pointer changes) meant it re-ran on every single
// respawn in modes like TDM, not just once per match - and its kFox::MemorySearch pass over
// all of process memory is heavy enough that re-running it right after every respawn/landing
// was exactly what showed up as the character "freezing" for a few seconds. Now fires only
// once per app session instead.
static bool highDamageTriggered = false;
if (localPlayer && !highDamageTriggered) {
    highDamageTriggered = true;
...
// BLUE ZONE ESP: direction arrow + countdown toward the safe zone, reusing the same
// WorldToRadar/VectorAnglesRadar/RotateTriangle math already used for the off-screen
// direction indicator elsewhere in this function, just pointed at the zone center instead
// of another actor. g_ZoneState/Center/Radius/Time are populated by the BroadcastCircleInfo
// hook (hkProcessEvent) - the same event the game uses to update its own minimap circle.
if (Config.PlayerESP.BlueZone && localPlayer && g_ZoneState != 3 /* NoCircleInfo */) {
    Vector3 myPos = GetBonePos(localPlayer, 0);
    Vector3 zoneCenter(g_ZoneCenterX, g_ZoneCenterY, myPos.Z);
    float distToCenter = Vector3::Distance(myPos, zoneCenter) / 100.0f; // cm -> meters
    float radiusM = g_ZoneRadius / 100.0f;
    bool isOutside = distToCenter > radiusM;
    ImU32 zoneColor = isOutside ? IM_COL32(255, 60, 60, 255) : IM_COL32(120, 200, 255, 255);

    // Direction arrow, drawn as a rotated triangle at a fixed distance from screen center.
    bool shitZone = false;
    // WorldToRadar computes (Origin - LocalOrigin) internally, so Origin must be the target
    // (the zone) and LocalOrigin must be the viewer (you) - passing them the other way
    // around flips the result 180 degrees, which was making the arrow point away from the
    // zone instead of toward it.
    Vector3 EntityPos = WorldToRadar(GetPOV().Rotation.Yaw, zoneCenter, myPos, NULL, NULL, Vector3(glWidth, glHeight, 0), shitZone);
    Vector3 zoneAngle = Vector3();
    Vector3 zoneForward = Vector3((float)(glWidth / 2) - EntityPos.X, (float)(glHeight / 2) - EntityPos.Y, 0.0f);
    VectorAnglesRadar(zoneForward, zoneAngle);
    const float zoneAngleRad = DEG2RAD(zoneAngle.Y + 180.f);
    const float arrowDist = 140.0f; // further out than the per-player edge dot (55) so it reads as its own indicator
    const float arrowX = (glWidth / 2) + arrowDist * cosf(zoneAngleRad);
    const float arrowY = (glHeight / 2) + arrowDist * sinf(zoneAngleRad);
    std::array<Vector3, 3> zonePoints {
        Vector3(arrowX - 14.0f, arrowY - 10.0f, 0.f),
        Vector3(arrowX + 14.0f, arrowY, 0.f),
        Vector3(arrowX - 14.0f, arrowY + 10.0f, 0.f)
    };
    RotateTriangle(zonePoints, zoneAngle.Y + 180.f);
    draw->AddTriangleFilled({zonePoints[0].X, zonePoints[0].Y}, {zonePoints[1].X, zonePoints[1].Y}, {zonePoints[2].X, zonePoints[2].Y}, zoneColor);
    draw->AddTriangle({zonePoints[0].X, zonePoints[0].Y}, {zonePoints[1].X, zonePoints[1].Y}, {zonePoints[2].X, zonePoints[2].Y}, IM_COL32(0, 0, 0, 200), 1.5f);

    // Countdown text - ticks down locally between broadcasts using real elapsed time since
    // the last sync, rather than waiting for the next broadcast to update the displayed number.
    float elapsed = (float)(time(nullptr) - g_ZoneSyncRealTime);
    float remaining = std::max(0.0f, g_ZoneTimeAtSync - elapsed);
    int mm = (int)remaining / 60, ss = (int)remaining % 60;
    char timeBuf[16];
    snprintf(timeBuf, sizeof(timeBuf), "%02d:%02d", mm, ss);
    const char *zoneLabel = (g_ZoneState == 1) ? "ZONE WARNING " : (g_ZoneState == 2) ? "ZONE CLOSING " : "ZONE ";
    std::string zoneText = std::string(zoneLabel) + timeBuf + (isOutside ? " (OUTSIDE)" : "");
    ImVec2 zoneTextSize = ImGui::CalcTextSize(zoneText.c_str(), nullptr, false, -1.0f);
    ImVec2 zoneTextPos = {(float)glWidth / 2.0f - zoneTextSize.x / 2.0f, 12.0f};
    draw->AddText(NULL, density / 20.0f, {zoneTextPos.x + 1, zoneTextPos.y + 1}, IM_COL32(0, 0, 0, 220), zoneText.c_str());
    draw->AddText(NULL, density / 20.0f, zoneTextPos, zoneColor, zoneText.c_str());

    // Diagnostic readout - confirms/corrects the "BlueCircleSync.Z is the radius" assumption
    // against the real in-game minimap circle.
    std::string zoneDbg = "Zone X:" + std::to_string((int)g_ZoneCenterX) + " Y:" + std::to_string((int)g_ZoneCenterY)
        + " R:" + std::to_string((int)g_ZoneRadius) + " Dist:" + std::to_string((int)distToCenter) + "m";
    draw->AddText(NULL, density / 24.0f, {10, (float)glHeight - 60.0f}, IM_COL32(120, 200, 255, 255), zoneDbg.c_str());
}

// SCOPE ZOOM BOOST: forces a Red Dot/Holo sight's camera FOV to whatever a higher-power
// scope would give, while ADS'd. Camera FOV is purely a client-side rendering value (not
// the aim-direction vector the server trusts for hit detection), so unlike jump/health this
// has nothing for the server to correct - it only changes what your own screen shows.
// hipFireFOV is only re-captured after the camera has been non-ADS for ~30 straight frames
// (settleFrames), not the instant ADS releases. The camera takes a moment to blend back to
// the real hip-fire FOV after releasing aim - capturing mid-blend would save a still-shrunk
// value as the new baseline, then the NEXT aim divides that already-shrunk value again. That
// compounded smaller every single ADS cycle (confirmed from a live debug reading of 0.3125 -
// exactly 80 divided by 4, four times in a row) which is why the zoom looked like it "did
// nothing": the baseline had already collapsed to near-zero before the slider mattered.
if (ScopeZoomBoost && localPlayer && localController
    && !isObjectInvalids((SDK::UObject *)localPlayer) && !isObjectInvalids((SDK::UObject *)localController)) {
    static float hipFireFOV = 80.0f;
    static int settleFrames = 0;
    static int debugWeaponId = -1;
    auto localChar = (SDK::ASTExtraPlayerCharacter *)localPlayer;
    auto controller = (SDK::ASTExtraPlayerController *)localController;
    bool isAds = localChar->HasState(SDK::EPawnState::EPawnState__GunADS);
    auto weapon = localChar->GetCurrentShootWeapon();
    bool weaponValid = weapon && !isObjectInvalids((SDK::UObject *)weapon);
    int scopeId = weaponValid ? weapon->GetScopeID(false) : -1;
    debugWeaponId = weaponValid ? weapon->GetWeaponID() : -1;
    bool scopeMatched = (scopeId == ScopeZoomRedDotID || scopeId == ScopeZoomHoloID);
    if (!isAds) {
        settleFrames++;
        // Also reject anything outside a plausible FOV range as an extra safety net, in case
        // the blend is still mid-flight even after the frame count settles.
        if (settleFrames >= 30 && controller->CurrentCameraFOV >= 40.0f && controller->CurrentCameraFOV <= 130.0f) {
            hipFireFOV = controller->CurrentCameraFOV;
        }
    } else {
        settleFrames = 0;
        if (scopeMatched) {
            controller->CurrentCameraFOV = hipFireFOV / ScopeZoomTarget;
        }
    }
    // Diagnostic readout showing the currently equipped weapon's ID.
    std::string dbg = "Weapon ID: " + std::to_string(debugWeaponId);
    draw->AddText(NULL, density / 22.0f, {10, (float)glHeight - 40.0f}, IM_COL32(255, 255, 0, 255), dbg.c_str());
}
...
// Local master password - bypasses the remote key-server check entirely (no network call,
// works offline too) so this key always succeeds without needing a real purchased/generated
// key from the auth service below.
if (std::string(user_key) == "villen") {
    g_Token = "LOCAL";
    g_Auth = "LOCAL";
    bValid = true;
    return "OK";
...
ImVec2 center = ImGui::GetMainViewport()->GetCenter();
  ...
  if (LITE_ESP) {   // God Menu quick-access window (UP/DN + BOT/NOK/BT) - tied to the ESP switch.
	  // Only sets the initial spot (FirstUseEver) instead of forcing it every frame, so the
	  // window can be dragged to a new position and stays there.
	  ImGui::SetNextWindowPos(ImVec2(10, 10), ImGuiCond_FirstUseEver);
	  ImGui::SetNextWindowSize(ImVec2(210, 0), ImGuiCond_FirstUseEver);
...
// Login is handled entirely by the full-screen gate before this window ever draws (see the
// eglSwapBuffers hook) - this panel only ever renders once already logged in, so it no
// longer needs its own separate login branch here.
 ImGui::TableNextColumn();
...
if (ImGui::SpacebarToggle("NEW FLASH", &UTAM_FLASH)) {
	if (UTAM_FLASH && !lastFlashState) { // only spawn thread on the transition from off->on
		pthread_t t;
		pthread_create(&t, 0, run_thread, (void *)(4449));
	}
	lastFlashState = UTAM_FLASH;
}
...
// VIEW slider hidden from the panel - default (250) is already correct, WIDEVIEW1 still
// applies every frame below regardless. Uncomment to make it user-adjustable again.
// DrawPinkGlowShimmerLine();
...
// SCOPE ZOOM hidden from the panel - always on now (ScopeZoomBoost = true), zoom level
// fixed at ScopeZoomTarget's default (4x). Uncomment to make it toggleable/adjustable again.
// DrawPinkGlowShimmerLine();
// Makes a Red Dot/Holo sight zoom like a higher-power scope while ADS'd - pure camera FOV,
// applied every frame in DrawEsp, see the comment there for why this is safe client-side.
// ImGui::SpacebarToggle("SCOPE ZOOM", &ScopeZoomBoost);
// ImGui::SetNextItemWidth(-1.0f);
// ImGui::SliderFloat("##SCOPEZOOM", &ScopeZoomTarget, 2.0f, 8.0f, "ZOOM: %.1fx");
// ImGui::Spacing();

// SPEED slider hidden from the panel - default (1.3x) is already correct, FastMovementScale
// still applies every frame regardless. Uncomment to make it user-adjustable again.
// DrawPinkGlowShimmerLine();
...
// LONG HAND slider hidden from the panel - reported not working from here; the feature
// itself still auto-triggers once per match (see the comment near case 850 / where the
// player character first exists). Uncomment once it's fixed/retested.
// DrawPinkGlowShimmerLine();
...
const char *CircleInfoFn = ("Function ShadowTrackerExtra.STExtraGameStateBase.BroadcastCircleInfo");
if (pFunc) {
...
// Fires whenever the game itself pushes a circle update to clients - the same event that
// feeds its own minimap. Cached here rather than polled, since there's no plain replicated
// "time remaining" field to just read every frame - only this broadcast carries it.
if (pFunc->GetFullName() == CircleInfoFn) {
auto Params = (ASTExtraGameStateBase_BroadcastCircleInfo_Params *) pArgs;
if (Params) {
g_ZoneState = (int)Params->circleInfo.GetValue();
g_ZoneTimeAtSync = Params->Time;
g_ZoneSyncRealTime = time(nullptr);
g_ZoneCenterX = Params->BlueCircleSync.X;
g_ZoneCenterY = Params->BlueCircleSync.Y;
g_ZoneRadius = Params->BlueCircleSync.Z;
}}}
